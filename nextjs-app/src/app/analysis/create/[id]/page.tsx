  ...
  const tryEmergencySave = async (results: SearchResult[], status: 'finished' | 'stopped') => {
    try {
      ...
      if (currentUser && currentUser.uid) {
        ...
        const { push, ref, set, get, query, orderByChild, equalTo } = await import('firebase/database');
        ...
        // Get the stored original query or a default
        const searchQuery = originalQueryRef.current || 'Search results';
        console.log(`Using query for emergency save: ${searchQuery}`);
        
        // Check if there's a recent analysis with the same query
        // This helps avoid creating duplicates
        try {
          // Get recent analyses from the last 2 minutes
          const twoMinutesAgo = Date.now() - 120000; // 2 minutes in milliseconds
          const userHistoryRef = ref(database, `users/${currentUser.uid}/history`);
          ...
          if (historySnapshot.exists()) {
            ...
            for (const historyId of historyIds) {
              const historyTimestamp = historySnapshot.val()[historyId];
              
              // Only check recent entries
              if (historyTimestamp > twoMinutesAgo) {
                const analysisRef = ref(database, `analyses/${historyId}`);
                const analysisSnapshot = await get(analysisRef);
                
                if (analysisSnapshot.exists()) {
                  const analysis = analysisSnapshot.val();
                  
                  // If we find a recent analysis with the same query
                  if (analysis.query === searchQuery) {
                    console.log(`Found existing analysis for query "${searchQuery}" with ID ${historyId}`);
                    alert('Your search results were already saved. Redirecting to the results page.');
                    window.location.href = `/analysis/finished/${historyId}`;
                    return;
                  }
                }
              }
          ...
          console.error('Error checking for existing analyses:', checkError);
          // Continue with emergency save anyway
        }
        
        // Create a new emergency analysis entry
        ...
        if (emergencyId) {
          ...
          try {
            const { cleanupDuplicateAnalyses } = await import('../../../services/analysisService');
            await cleanupDuplicateAnalyses(currentUser.uid);
          } catch (cleanupError) {
